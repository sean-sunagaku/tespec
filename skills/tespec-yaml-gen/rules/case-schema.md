...
## YAML 特殊文字の注意
値に以下のパターンが含まれると構文エラーになる。クォートで囲むか表現を変えて回避する。

| パターン | 問題 | 対処法 |
|---------|------|--------|
| `*` で始まる値 | アンカー参照と誤認 | 表現を変えるか `"*foo"` とクォート |
| `"..."` を含む値 | クォートの入れ子 | 外側を `'` で囲むか表現を変える |
| `#` を含む値 | コメントと誤認 | `"値に#を含む"` とクォート |
| `: ` を含む値 | キーと誤認 | クォートで囲む |

```yaml
# BAD
- action: *Tests.swift を探す
# GOOD
- action: Tests.swift パターンを探す
```
